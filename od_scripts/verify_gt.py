...
SOURCE_LABEL = {"real": "LOCO real (train)", "valid": "LOCO subset-3 (valid)", "basev2": "base_v2 synth", "may": "may-rounds synth", "traj_optuna": "trajectory-optimized synth", "basev4": "base_v4 synth (traj+random)"}
THUMB_W = 520
PER_SOURCE = 2
...
def main():
    ...
    parts.append("<h2>Composition</h2><div class='tblwrap'><table><tr><th>dataset</th><th>role</th><th class='n'>images</th>"
                 "<th class='n'>real</th><th class='n'>base_v2</th><th class='n'>may</th><th class='n'>traj-opt</th><th class='n'>base_v4</th>"
    ...
    parts.append(f"<tr><td>real/valid (shared by all arms)</td><td>validation</td><td class='n'>{v['images']}</td><td class='n'>{v['images']}</td>"
                 f"<td class='n'>0</td><td class='n'>0</td><td class='n'>0</td><td class='n'>0</td>" + "".join(f"<td class='n'>{v['class_counts'].get(c, 0)}</td>" for c in classes) + "</tr>")
    for arm, d in manifest["arms"].items():
        ...
        parts.append(f"<tr><td>{arm}/train</td><td>training arm</td><td class='n'>{d['train_images']}</td><td class='n'>{bs.get('real', 0)}</td>"
                     f"<td class='n'>{bs.get('basev2', 0)}</td><td class='n'>{bs.get('may', 0)}</td><td class='n'>{bs.get('traj_optuna', 0)}</td><td class='n'>{bs.get('basev4', 0)}</td>"
    ...
    for s in ("real", "valid", "basev2", "may", "traj_optuna", "basev4"):
        d = st[s]
