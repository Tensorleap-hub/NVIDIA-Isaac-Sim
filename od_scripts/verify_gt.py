...
SOURCE_LABEL = {"real": "LOCO real (train)", "valid": "LOCO subset-3 (valid)", "basev2": "base_v2 synth", "may": "may-rounds synth", "traj_optuna": "trajectory-optimized synth", "basev4": "base_v4 synth (traj+random)", "optuna_rand": "optuna-rand synth"}
THUMB_W = 520
PER_SOURCE = 2
...
def main():
    ...
    parts.append("<h2>Composition</h2><div class='tblwrap'><table><tr><th>dataset</th><th>role</th><th class='n'>images</th>"
                 "<th class='n'>real</th><th class='n'>base_v2</th><th class='n'>may</th><th class='n'>traj-opt</th><th class='n'>base_v4</th><th class='n'>opt-rand</th>"
    ...
    parts.append(f"<tr><td>real/valid (shared by all arms)</td><td>validation</td><td class='n'>{v['images']}</td><td class='n'>{v['images']}</td>"
                 f"<td class='n'>0</td><td class='n'>0</td><td class='n'>0</td><td class='n'>0</td><td class='n'>0</td>" + "".join(f"<td class='n'>{v['class_counts'].get(c, 0)}</td>" for c in classes) + "</tr>")
    for arm, d in manifest["arms"].items():
        ...
        parts.append(f"<tr><td>{arm}/train</td><td>training arm</td><td class='n'>{d['train_images']}</td><td class='n'>{bs.get('real', 0)}</td>"
                     f"<td class='n'>{bs.get('basev2', 0)}</td><td class='n'>{bs.get('may', 0)}</td><td class='n'>{bs.get('traj_optuna', 0)}</td><td class='n'>{bs.get('basev4', 0)}</td><td class='n'>{bs.get('optuna_rand', 0)}</td>"
                     + "".join(f"<td class='n'>{d['class_counts'].get(c, 0)}</td>" for c in classes) + "</tr>")
    parts.append("</table></div>")

    # ---- per-source box stats: every source computed independently from its own evalset
    # (a source not in real_all_traj, e.g. optuna_rand, would otherwise silently drop out)
    c_all, id2n, anns_all = load(OUT / "real_all_traj" / "train")
    st = {"real": stats(c_all["images"], anns_all, id2n)["real"]}
    for s in ("basev2", "may", "traj_optuna", "basev4", "optuna_rand"):
        es = OUT / "evalsets" / f"train_{s}" / "valid"
        if es.exists():
            c_s, id2n_s, anns_s = load(es)
            st[s] = stats(c_s["images"], anns_s, id2n_s).get(s, {"images": 0, "boxes": {}, "median_rel_area": {}})
    ...
    for s in ("real", "valid", "basev2", "may", "traj_optuna", "basev4", "optuna_rand"):
        d = st.get(s, {"images": 0, "boxes": {}, "median_rel_area": {}})
        if d["images"] == 0:
            continue
