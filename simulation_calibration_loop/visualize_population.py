...
from simulation_calibration_loop.data import DINOv2Embedder, RFDETREmbedder, make_cache_key, select_real_image_paths

AnyEmbedder = DINOv2Embedder | RFDETREmbedder
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--backend", choices=["dinov2", "rfdetr"], default="dinov2")
    parser.add_argument("--checkpoint", type=Path, default=None, help="RF-DETR checkpoint path (required for --backend rfdetr)")
    parser.add_argument("--num-classes", type=int, default=3, help="Number of classes the RF-DETR checkpoint was trained with")
    parser.add_argument(
        "--extraction-mode",
        choices=["encoder", "neck", "decoder"],
        default="encoder",
        help="encoder: raw ViT backbone GAP; neck: projected transformer input tokens; decoder: last decoder query outputs",
    )
...
def resolve_runtime_config(args: argparse.Namespace) -> tuple[Path, Path, DINOv2Config]:
    ...
    if args.backend == "rfdetr":
        if args.checkpoint is None:
            raise ValueError("--checkpoint is required when --backend rfdetr")
        ckpt_stem = Path(args.checkpoint).stem
        dino.model_name = f"rfdetr_{args.extraction_mode}_{ckpt_stem}"
        dino.repo = ""
    else:
...
def load_or_compute_image_embeddings(
    *,
    ...
    if embedder is None:
        raise ValueError(f"Cannot compute {label} embeddings without an embedder")
...
def discover_selected_runs(
    *,
    ...
    if not cache_paths:
        if allow_empty:
            return []
        raise ValueError(f"No selected cache files found under {selected_root}")
...
def print_discovery_summary(
    *,
    ...
    model_id = f"{dino.repo}:{dino.model_name}" if dino.repo else dino.model_name
    print(f"Embedder model: {model_id}")
...
def main() -> int:
    ...
    selected_runs = discover_selected_runs(
        ...
        allow_empty=bool(uncached_run_specs),
    )
    ...
    if uncached_run_specs and embedder is not None:
        print(f"Computing embeddings for {len(uncached_run_specs)} uncached output runs...")
